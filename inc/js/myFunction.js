...
function findChiSoByTuyChon(array, tuy_chon) {
    ...
    return rs != undefined ? parseInt(rs.data) : undefined;
}
...
function dataLine(myObj, parent, ten_don_vi, tuy_chon) {
    let data;
    let dataPoint;
    switch (tuy_chon) {
        case 'chart3':
            data = myObj.filter(e => {
                return e.parent == parent && e.ten_don_vi == ten_don_vi && e.tuy_chon == 'tong-so-viec-can-hoan-thanh-trong-thang';
            })
            data = groupBy(data, 'thang')
            dataPoint = [];
            for (const thoigian in data) {
                dataPoint.push({
                    y: Number(data[thoigian][0].data),
                    x: parseInt(thoigian),
                    name: 'Tháng ' + thoigian,
                })
            }
        ...
        case 'chart4':
            data = myObj.filter(e => {
                return e.parent == parent && e.ten_don_vi == ten_don_vi && e.tuy_chon == 'viec-cham-chua-hoan-thanh';
            })
            data = groupBy(data, 'thang')
            dataPoint = [];
            for (const thoigian in data) {
                let sum = myObj.find(element => {
                    return element.ten_don_vi == ten_don_vi && element.parent == parent && element.thang == thoigian && element.tuy_chon == 'tong-so-viec-can-hoan-thanh-trong-thang'
                }).data;
                dataPoint.push({
                    y: parseInt(data[thoigian][0].data) / parseInt(sum) * 100,
                    x: parseInt(thoigian),
                    name: 'Tháng ' + thoigian,
                })
            }
        ...
        case 'chart5':
            data = myObj.filter(e => {
                return e.parent == parent && e.ten_don_vi == ten_don_vi && (e.tuy_chon == 'viec-hoan-thanh-khong-dung-han-co-due-date-trong-thang' || e.tuy_chon == 'viec-cham-chua-hoan-thanh-co-due-date-trong-thang' || e.tuy_chon == 'viec-co-due-date-trong-thang')
            })
            data = groupBy(data, 'thang')
            dataPoint = [];
            for (const thoigian in data) {
                let ty_le = Math.round((findChiSoByTuyChon(data[thoigian], 'viec-hoan-thanh-khong-dung-han-co-due-date-trong-thang') + findChiSoByTuyChon(data[thoigian], 'viec-cham-chua-hoan-thanh-co-due-date-trong-thang')) / findChiSoByTuyChon(data[thoigian], 'viec-co-due-date-trong-thang') * 100)
                dataPoint.push({
                    y: ty_le,
                    x: parseInt(thoigian),
                    name: 'Tháng ' + thoigian,
        ...
        case 'chart6':
            data = myObj.filter(e => {
                return e.parent == parent && e.ten_don_vi == ten_don_vi && (e.tuy_chon == 'viec-con-lai-cua-cac-thang-truoc-duoc-hoan-thanh-trong-thang' || e.tuy_chon == 'viec-con-lai-cua-cac-thang-truoc')
            })
            data = groupBy(data, 'thang')
            dataPoint = [];
            for (const thoigian in data) {
                let ty_le = Math.round(findChiSoByTuyChon(data[thoigian], 'viec-con-lai-cua-cac-thang-truoc-duoc-hoan-thanh-trong-thang') / findChiSoByTuyChon(data[thoigian], 'viec-con-lai-cua-cac-thang-truoc') * 100)
                dataPoint.push({
                    y: ty_le,
                    x: parseInt(thoigian),
                    name: 'Tháng ' + thoigian,
        ...
        case 'chart7':
            data = myObj.filter(e => {
                return e.parent == parent && e.ten_don_vi == ten_don_vi && (e.tuy_chon == 'viec-hoan-thanh-dung-han-co-due-date-trong-thang' || e.tuy_chon == 'viec-hoan-thanh-khong-dung-han-co-due-date-trong-thang' || e.tuy_chon == 'viec-con-lai-cua-cac-thang-truoc-duoc-hoan-thanh-trong-thang')
            })
            data = groupBy(data, 'thang')
            dataPoint = [];
            for (const thoigian in data) {
                let ty_le = Math.round((findChiSoByTuyChon(data[thoigian], 'viec-hoan-thanh-dung-han-co-due-date-trong-thang') + findChiSoByTuyChon(data[thoigian], 'viec-hoan-thanh-khong-dung-han-co-due-date-trong-thang') + findChiSoByTuyChon(data[thoigian], 'viec-con-lai-cua-cac-thang-truoc-duoc-hoan-thanh-trong-thang')) / data[thoigian][0].count_user)
                dataPoint.push({
                    y: ty_le,
                    x: parseInt(thoigian),
                    name: 'Tháng ' + thoigian,
        ...
        case 'chart8':
            data = myObj.filter(e => {
                return e.parent == parent && e.ten_don_vi == ten_don_vi && (e.tuy_chon == 'viec-chuyen-sang-dang-thuc-hien-nho-hon-2-ngay' || e.tuy_chon == 'viec-co-due-date-trong-thang')
            })
            data = groupBy(data, 'thang')
            dataPoint = [];
            for (const thoigian in data) {
                let ty_le = Math.round(findChiSoByTuyChon(data[thoigian], 'viec-chuyen-sang-dang-thuc-hien-nho-hon-2-ngay') / findChiSoByTuyChon(data[thoigian], 'viec-co-due-date-trong-thang') * 100)
                if (!isNaN(ty_le)) {
                    dataPoint.push({
                        y: ty_le,
                        x: parseInt(thoigian),
        ...
        case 'chart9':
            data = myObj.filter(e => {
                return e.parent == parent && e.ten_don_vi == ten_don_vi && (e.tuy_chon == 'viec-cho-duyet-qua-3-ngay' || e.tuy_chon == 'tong_so_viec_cho_duyet_trong_thang')
            })
            data = groupBy(data, 'thang')
            dataPoint = [];
            for (const thoigian in data) {
                let ty_le = Math.round(findChiSoByTuyChon(data[thoigian], 'viec-cho-duyet-qua-3-ngay') / findChiSoByTuyChon(data[thoigian], 'tong_so_viec_cho_duyet_trong_thang') * 100)
                dataPoint.push({
                    y: ty_le,
                    x: parseInt(thoigian),
                    name: 'Tháng ' + thoigian,
